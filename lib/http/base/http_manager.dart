...
class HttpManager {
    ...
    if (_client == null) {
      ...
      BaseOptions options = BaseOptions(
        ...
        headers: {
          ...
          HttpHeaders.userAgentHeader: "DXiOS",
        },
  ...
  Future<T> _requestAsync<T>({
    ...
    var t = (DateTime.now().millisecondsSinceEpoch/1000.0).toStringAsFixed(0);
    params["t"] = t;
    ///排序
    var keyQueue = <String>[];
    var signParams = <String,dynamic>{};
    keyQueue.addAll(params.keys);
    keyQueue.sort((obj1,obj2) => obj1.compareTo(obj2));
    keyQueue.forEach((element) {
      signParams[element]=params[element];
    });
    LogUtil.v("signParams"+signParams.toString());
    var jsonString = json.encode(signParams) + t + "bd0b51346578853fc0977bf71a4b7e3d";
    var content = new Utf8Encoder().convert(jsonString);
    params["sign"] = hex.encode(md5.convert(content).bytes);
    LogUtil.v(params);
    try {
      CancelToken cancelToken;
      if (tag != null) {
      ...
      Response<T> response = await _client.request(url,
          queryParameters: params,
          data: data,
          options: options,
          cancelToken: cancelToken);
      ...
      try {
        ...
        dataJson = json.decode(dataString);
      } catch (e, s) {
        dataJson = response.data as Map<String, dynamic>;
      }
      int statusCode = dataJson['return']["code"];
      if (statusCode == 0) {
        //成功
        if (jsonParse != null) {
          return jsonParse(dataJson["data"]);
        } else {
          return dataJson["data"];
        }
      } else {
        //失败
        String message = dataJson['return']["desc"];
        LogUtil.v("请求服务器出错：$message");
  ...
  Future<T> uploadAsync<T>({
    ...
    try {
      ...
      String statusCode = response.data["result"]["code"];
      if (statusCode == "0") {
        //成功
        if (jsonParse != null) {
